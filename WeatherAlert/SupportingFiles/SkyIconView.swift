...
//  WeatherAlert
//
//  Created by Shane Nelson on 10/17/17.
...
class SKYIconView: UIView
{
  ...
  fileprivate func leaf(_ ctx: CGContext, t: Double, x: CGFloat, y: CGFloat, cw:CGFloat, s: CGFloat, color: CGColor)
  {
    ...
    ctx.beginPath()
    CGContextAddArc(ctx, x, y, a, d, d + CGFloat.pi, 1)
    CGContextAddArc(ctx, x - b * e, y - b * f, c, d + CGFloat.pi, d, 1)
    CGContextAddArc(ctx, x + c * e, y + c * f, b, d + CGFloat.pi, d, 0)
  ...
  {
    ctx.beginPath()
    CGContextAddArc(ctx, x, y, r, 0, CGFloat.pi * 2, 1)
    ctx.fillPath()
  }
